...
> rgringotts -p 7978 -h localhost -f $HOME/storagesecret/mygringottsfile
...
|--------|------|------|---------|-------------|
| `POST` | `/api/session/open` | `{"file":"…","passphrase":"…"}` | `201` `{"token":"<uuid>"}` | Decrypt the gringotts file and open a session. |
...
TOKEN=$(curl -s -X POST http://localhost:7979/api/session/open \
  ...
  -d '{"file":"/home/user/.gringotts/notes.grg","passphrase":"s3cr3t"}' \
  | jq -r .token)
...
# 5. Update it (id returned by the create call, e.g. 1)
curl -s -X PUT http://localhost:7979/api/entries/1 \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"title":"Bank PIN","body":"5678"}' | jq

# 6. Keep the session alive while idle
